...
class ZhihuSpider extends BaseSpider {
  ...
  async afterInputEditor() {
    ...
    await this.page.evaluate(() => {
      const el = document.querySelector('.PublishPanel-triggerButton') as HTMLInputElement
      el.click()
    })
    await this.page.waitForTimeout(5000)
    ...
    // 点击下一步
    await this.page.evaluate(() => {
      const el = document.querySelector('.PublishPanel-stepOneButton > button') as HTMLDivElement
      el.click()
    })
  ...
  async publish() {
    ...
    try {
      await this.page.evaluate(() => {
        const el = document.querySelector('.PublishPanel-stepTwoButton') as HTMLDivElement
        el.click()
      })
    } catch (e) {
      // do nothing
